...
| 优先级 | 项目 | 状态 | 理由 |
|--------|------|------|------|
| P0 | Review 分级决策 | **已完成** | 0-100 分数，≥90 通过，70-89 带条件通过，<70 重做 |
| P0 | Preflight check | **已完成** | feasibility=low 时自动 BLOCKED |
| P1 | 版本存档 | **已完成** | .versions/ 目录保存每轮完整产出（替代增量迭代方案） |
| P1 | Checkpoint 机制 | 待做 | 长任务可靠性 |
| P1 | 优雅停止 | 待做 | 基本可用性 |
| P2 | Reviewer prompt 分离 | 待做 | 质量改进，不紧急 |
